...
from agentic_payments_env.benchmark.runner import _drive, run_benchmark, tasks_for_benchmark
...
def _reward_spec_markdown() -> str:
    """Render the reward spec with examples evaluated from current code. REQ-ENV-14."""
    routine = load_task("v0/rt-001")
    routine_env, routine_trace = _drive(routine, build("oracle", routine), 0)
    routine_result = grade_episode(routine, routine_trace, routine_env.state)
    policy = load_task("v0/pc-001")
    policy_env, policy_trace = _drive(policy, build("liar", policy), 0)
    policy_result = grade_episode(policy, policy_trace, policy_env.state)
    weight_rows = "\n".join(
        f"| {dimension.value} | {weight} |" for dimension, weight in WEIGHTS.items()
    )
    return f"""# Reward specification (v1)
...
- oracle on `v0/rt-001`: `episode_reward={episode_reward(routine_result)}`;
  `step_rewards={step_rewards(routine_trace)}`.
- liar on `v0/pc-001`: `episode_reward={episode_reward(policy_result)}`;
  `catastrophic_codes={policy_result.catastrophic_codes}`.
...
def rebuild(out_dir: Path) -> None:
    ...
    (reward_out / "reward-spec.md").write_text(_reward_spec_markdown(), encoding="utf-8")
...
def check(out_dir: Path) -> None:
    ...
    _compare_text(
        root / "reports" / "v1" / "reward-spec.md",
        out_dir / "reports" / "v1" / "reward-spec.md",
