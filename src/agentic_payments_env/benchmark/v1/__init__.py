"""v1 held-out task registry loaded from frozen JSON. REQ-TASK-02."""
...
def _frozen_dir() -> Path:
    return Path(__file__).resolve().parents[4] / "benchmarks" / "v1"


def load_task(task_id: str) -> TaskSpec:
    """Load a frozen v1 TaskSpec. REQ-TASK-02."""
    if not task_id.startswith("v1/"):
...
def all_tasks() -> list[TaskSpec]:
    """Every frozen v1 held-out task, sorted by task_id."""
