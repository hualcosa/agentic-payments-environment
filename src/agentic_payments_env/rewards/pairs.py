"""Preference ranking and export from episode results. M4 T4.03."""
...
def assert_training_task_id(task_id: str) -> None:
    """Reject v1/v1.1 held-out ids for training exports."""
...
def training_task_ids() -> list[str]:
    """v0 tasks plus v1.1-train; never held-out v1 or v1.1."""
...
def build_preference_records(
    ...
    *,
    ...
    """Build preference rows; ties are omitted deterministically. T4.03."""
...
def dumps_preferences(records: list[PreferenceRecord]) -> str:
    """Canonical JSONL text for preference records."""
    lines = [
...
def export_preferences(out_path: Path, task_ids: list[str] | None = None) -> None:
    """Write preference JSONL for training tasks only. T4.03."""
