...
> **Port note (2026-09-21).** The audit ran on `composer25`, a parallel M0–M6
> implementation. On this `main` history the tickets were ported one commit per
> ticket from `composer25@7ab9285`, resolving against main's code; derived
> artifacts (v1.1 freeze, datasets, reports) were regenerated from main's code,
> never copied. Decision numbers here follow main's `docs/12-decisions.md`
> (composer25 D-17..D-25 are main D-19..D-25; main's Q-06 is the adv-001
> nickname question, so the Q-06 bullet below maps to D-25 only). Task:
> `.coordination/tasks/in-progress/20260918-001-port-m7-conformance.md`.
